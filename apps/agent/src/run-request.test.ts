...
describe("validateRunRequest", () => {
  ...
  it("rejects a target URL outside the app origin", () => {
    ...
    expect(result).toEqual({ ok: false, error: "targetUrl origin is not allowed" })
  })
  ...
  it("rejects a target URL outside the allowed report paths", () => {
    ...
    expect(result).toEqual({
      ok: false,
      error: "targetUrl path must be one of: /api/settlement-risk-report, /api/premium-risk-report",
    })
  })
  ...
  it("accepts allowed report paths", () => {
    ...
    if (result.ok) {
      ...
      expect(result.value.targetUrl).toBe(`${APP_URL}/api/settlement-risk-report`)
    }
  })
})
