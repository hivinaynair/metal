...
export function buildTraceSteps(result: {
  ...
  if (!result) {
    ...
    return [
      { id: 1, label: "402 Challenge", status: animStep > 1 ? "approved" : animStep === 1 ? "running" : "pending" },
      { id: 2, label: "ERC-8004 Identity", status: animStep > 2 ? "approved" : animStep === 2 ? "running" : "pending" },
      { id: 3, label: "AP2 Mandate", status: animStep > 3 ? "approved" : animStep === 3 ? "running" : "pending" },
      { id: 4, label: "Policy Check", status: animStep > 4 ? "approved" : animStep === 4 ? "running" : "pending" },
      { id: 5, label: "Settlement + Attestation", status: animStep === 5 ? "running" : "pending" },
    ]
  }
  ...
  const txShort = result.txHash ? `${result.txHash.slice(0, 10)}…` : undefined
  ...
  return [
    ...
    },
    {
      ...
      detail: txShort,
      link: result.txHash
        ? { href: `${BASE_SEPOLIA_EXPLORER}/tx/${result.txHash}`, label: "settlement tx" }
        : undefined,
    },
  ]
}
